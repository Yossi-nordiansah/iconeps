...
export const fetchDashboardData = createAsyncThunk(
  ...
  async (periode) => {
    const res = await axios.get(`/api/pusbas/dashboard?periode=${encodeURIComponent(periode)}`);
    return res.data;
  }
);

const dashboardSlicePusbas = createSlice({ 
  name: 'dashboard',
  initialState: {
    jumlahMahasiswa: 0,
    jumlahKelas: 0,
...
export default dashboardSlicePusbas.reducer;
