...
export default function AdminCetakLaporan() {
    ...
    const handlePreviewLaporan = async () => {
        ...
        try {
            const res = await fetch('/api/puskom/laporan/cetak', {
                method: 'POST',
    ...
    return (
        <div className="relative pl-52 pr-6 pt-20">
            ...
            <button
                ...
                disabled={!excelBlob}
                className={`bg-[#39ac73] text-white font-semibold rounded-sm hover:bg-[#40bf80] px-3 py-2 mx-auto mt-4 flex items-center justify-center gap-2 transition ${!excelBlob ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
            >
                <Download size={18} />
